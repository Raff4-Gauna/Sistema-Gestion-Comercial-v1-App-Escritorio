...
namespace CapaPresentación.MdInventarios
{
    public partial class frmMargenGanancias : Form
    {
        ...
        private void frmMargenGanancias_Load(object sender, EventArgs e)
        {
            ...
            //Mostrar todos los margenes

            List<Margenes_Ganancias> Lista = new CN_Margenes_Ganancias().Listar();

            foreach (Margenes_Ganancias item in Lista)
            {

                dgvdata.Rows.Add(new object[] {"", item.IdMargenGanancia, item.NombreMargen, item.Porcentaje,
        ...
        private void btnguardar_Click(object sender, EventArgs e)
        {
            ...
            Margenes_Ganancias obj = new Margenes_Ganancias()
            {
            ...
            if (obj.IdMargenGanancia == 0)
            {

                int idgenerado = new CN_Margenes_Ganancias().Registrar(obj, out mensaje);

                if (idgenerado != 0)
                {
                    dgvdata.Rows.Add(new object[] {"", idgenerado, txtNombreMargen.Text, txtPorcentaje.Text,
            ...
            {
                bool resultado = new CN_Margenes_Ganancias().Editar(obj, out mensaje);

                if (resultado)
                {
                    DataGridViewRow row = dgvdata.Rows[Convert.ToInt32(txtindice.Text)];
                    row.Cells["Id"].Value = txtid.Text;
                    row.Cells["NombreMargen"].Value = txtNombreMargen.Text;
        ...
        private void Limpiar()
        {
            txtindice.Text = "-1";
            txtid.Text = "0";
            txtNombreMargen.Text = "";
            txtPorcentaje.Text = "";
            cboestado.SelectedIndex = 0;
            txtNombreMargen.Select();
        }
        ...
        private void dgvdata_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvdata.Columns[e.ColumnIndex].Name == "btnseleccionar")
            {
                ...
                if (indice >= 0)
                {
                    ...
                    txtNombreMargen.Text = dgvdata.Rows[indice].Cells["NombreMargen"].Value.ToString();
        ...
        private void btneliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MessageBox.Show("¿Desea eliminar el margen de ganancias", "Mensaje", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    string mensaje = string.Empty;
                    Margenes_Ganancias obj = new Margenes_Ganancias()
                    {
                    ...
                    bool respuesta = new CN_Margenes_Ganancias().Eliminar(obj, out mensaje);
